...
import DepthOptimizedRoom from './DepthOptimizedRoom';
import NavigationUI from './NavigationUI';
import LoadingSpinner from './LoadingSpinner';
import OptimizedImmersiveControls from './OptimizedImmersiveControls';

interface ImmersiveMuseumSceneProps {
  onRoomChange?: (roomId: number) => void;
}

export default function ImmersiveMuseumScene({ onRoomChange }: ImmersiveMuseumSceneProps) {
  ...
  // Position de caméra différente selon si on est immergé ou non
  const getCameraPosition = () => {
    if (isImmersed) {
      return [0, 0, -80]; // Encore plus reculé pour une vue plus large
    }
    return [0, 15, 25]; // Vue externe plus éloignée
  };
