...
interface CorrectedImmersiveRoomProps {
  room: MuseumRoom;
  isActive: boolean;
  isVisited: boolean;
...
export default function CorrectedImmersiveRoom({
  room,
  isActive,
  isVisited,
  isTransitioning,
  onRoomClick
}: CorrectedImmersiveRoomProps) {
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (loadedTexture) => {
        try {
          // Configuration correcte pour éviter l'inversion
          loadedTexture.wrapS = loadedTexture.wrapT = 1000; // RepeatWrapping
          loadedTexture.flipY = true; // Corrigé pour éviter l'inversion
